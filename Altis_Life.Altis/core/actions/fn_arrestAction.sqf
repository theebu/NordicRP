...
_time = param [ 1 , 30 ]; // that's new!
if (isNull _unit) exitWith {}; //Not valid
if (isNil "_unit") exitWith {}; //Not Valid
if (!(_unit isKindOf "Man")) exitWith {}; //Not a unit
if (!isPlayer _unit) exitWith {}; //Not a human
if (!(_unit getVariable "restrained")) exitWith {}; //He's not restrained.
if ( ! ((side _unit) in [civilian, independent, east])) exit with {}; // Here I have added the Group East, which is important for my project
if (isNull _unit) exitWith {}; //Not valid
if (life_HC_isActive) then {
 if (_time <  1 ) exit with {}; // this query has been added.
 [getPlayerUID _unit,_unit,player,false] remoteExecCall ["HC_fnc_wantedBounty",HC_Life];
} else {
 if (_time <  1 ) exit with {}; // as this query.
...
*/ 
// The upper I excluded. Did not want to delete it.
