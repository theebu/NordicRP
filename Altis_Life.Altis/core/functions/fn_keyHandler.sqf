...
switch (_code) do {
    ...
    case 19: {
		...
		if (_shift && playerSide isEqualTo civilian && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject in [west,civilian,independent])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {(cursorObject getVariable "Incapacitated")} && {!(cursorObject getVariable "Escorting")} && {!(cursorObject getVariable "restrained")} && {speed cursorObject < 1}) then {
            [] call life_fnc_restrainAction;
        };
    };
	...
	// J, police gate opener
    case 36:
	{
		if(_shift) then {_handled = true;};

		if (_shift && !_alt && !_ctrlKey && (playerSide == west || license_civ_rebel || playerSide == independent) && (vehicle player != player)) then {
			[] call life_fnc_copOpener;
		};
		if (_shift && !_alt && !_ctrlKey && (license_civ_tow) && (vehicle player != player)) then {
			[] call life_fnc_towgateOpener;
		};
		if (_shift && !_alt && !_ctrlKey && (license_civ_admin)) then {
			[] call life_fnc_adminOpener;
		};
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_blue_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_blue_F","B_Heli_Light_01_F","I_Heli_light_03_unarmed_F","C_Offroad_01_Red_F","C_Heli_light_01_red_F"]) then {
	...
	case 79: 
	{
		if((!_shift) && playerSide in [west] && {vehicle player != player} && {!life_yelp_active} && {((driver vehicle player) == player)}) then {
	    	[] spawn {
	...
	case 80: 
	{
		if((!_shift) && playerSide in [west] && {vehicle player != player} && {!life_yelp_active} && {((driver vehicle player) == player)}) then {
	        ...
	        _veh = vehicle player;
	        [_veh] remoteExec ["life_fnc_copMeg1",RCLIENT];
		};
	};
	...
	case 81: 
	{
		if((!_shift) && playerSide in [west] && {vehicle player != player} && {!life_yelp_active} && {((driver vehicle player) == player)}) then {
	        ...
	        _veh = vehicle player;
	        [_veh] remoteExec ["life_fnc_copMeg2",RCLIENT];
		};
	};
	...
	case 75: 
	{
		if((!_shift) && playerSide in [west] && {vehicle player != player} && {!life_yelp_active} && {((driver vehicle player) == player)}) then {
	        ...
	        _veh = vehicle player;
	       [_veh] remoteExec ["life_fnc_copMeg3",RCLIENT];
		};
	};
	...
	case 76: 
	{
		if((!_shift) && playerSide in [west] && {vehicle player != player} && {!life_yelp_active} && {((driver vehicle player) == player)}) then {
			...
			_veh = vehicle player;
	        [_veh] remoteExec ["life_fnc_copMeg4",RCLIENT];
		};
	};
